...
EchoServer::EchoServer(std::string ip,uint16_t port,int threadnum,int workThreadNum)
            :tcpServer_(ip,port,threadnum),tp_(workThreadNum,"WORK"),workThreadNum_(threadnum){
...
void EchoServer::handleNewConnection(Socket *cliSocket){
     printf("[SERVER] : Client Connect Success, Ip = %s, Port = %hu, ThreadId = %ld\n",cliSocket->getAddr()->ip(),cliSocket->getAddr()->port(),syscall(SYS_gettid));
}
...
void EchoServer::handleOnMessageSendComplete(Connection *con){
    printf("[SERVER] : Connection %s:%d Send Complete, ThreadId = %ld\n",con->getCliSocket()->getAddr()->ip(),con->getCliSocket()->getAddr()->port(),syscall(SYS_gettid));
}
...
//用于处理客户端业务（工作线程）
void EchoServer::onMessage(Connection *con,std::string &data){
