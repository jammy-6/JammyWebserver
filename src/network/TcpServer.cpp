...
TcpServer::TcpServer(std::string ip, uint16_t port, TYPE type,int threadnum)
    : mainLoops_(new EventLoop(true, 3, 5)), threadnum_(threadnum),
      thp_(new ThreadPool(threadnum, "IO")),
      acceptor_(new Acceptor(ip, port, mainLoops_.get())),type_(type) {
...
void TcpServer::newConnection(Socket *cliSocket) {
  ...
  spConnection cliConnection(
      new Connection(subLoops_[cliSocket->fd() % threadnum_].get(), cliSocket,type_));
