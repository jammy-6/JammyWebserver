...
Buffer::Buffer(int initBuffSize)
    : buff_(initBuffSize, 0), readPos_(0), writePos_(0) {}

size_t Buffer::ReadableBytes() const { return writePos_ - readPos_; }
size_t Buffer::WritableBytes() const { return buff_.size() - writePos_; }
...
char *Buffer::BeginPtr_() { return &*buff_.begin(); }

const char *Buffer::BeginPtr_() const { return &*buff_.begin(); }
...
// 往buffer中添加数据
// void Buffer::append(const char *data, int size) { buff_.append(data, size); }
// // 返回buff大小
// int Buffer::size() { return buff_.size(); }
// // 返回首地址指针
// const char *Buffer::ptr() { return buff_.data(); }

// void Buffer::clear() { buff_.clear(); }

// void Buffer::pop(int size) { buff_.erase(0, size); }

bool Buffer::getMsg(std::string &msg) {
  if (ReadableBytes() >= 4) {
    int len;
    memcpy(&len, Peek(), sizeof(int));
    if (ReadableBytes() >= 4 + len) {
      msg = std::string(Peek() + 4, len);
      Retrieve(4 + len);
      return true;
    }
  }
...
std::string &Buffer::getStr() { return buff_; }