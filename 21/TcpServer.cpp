...
TcpServer::TcpServer(std::string ip,uint16_t port,int threadnum)
                                                            :mainLoops_(new EpollLoop(true,3,5)),
                                                            ...
                                                            acceptor_ (new Acceptor(ip,port,mainLoops_.get()))
{
    ...
    mainLoops_->setEpollTimeOutCallback(std::bind(&TcpServer::onEpollTimeOut,this,mainLoops_.get()));

    //往线程池中添加任务
    for(int i=0;i<threadnum;i++){
        subLoops_.emplace_back(new EpollLoop(false,5,5));
        subLoops_[i]->setEpollTimeOutCallback(std::bind(&TcpServer::onEpollTimeOut,this,mainLoops_.get()));
        subLoops_[i]->setConnTimeoutCallBack(std::bind(&TcpServer::removeConnect,this,std::placeholders::_1));
...
void TcpServer::newConnection(Socket *cliSocket){
    ...
    //在从事件循环中分配客户端连接
    spConnection cliConnection(new Connection(subLoops_[cliSocket->fd()%threadnum_].get(),cliSocket));
    ...
    subLoops_[cliSocket->fd()%threadnum_]->appendConn(cliSocket,cliConnection);//将连接添加进EpollLoop中的map
...
void TcpServer::setOnMessageCallback(std::function<void(spConnection con,std::string &data)> fun){
    onMessageCallback_ = fun;
}
void TcpServer::setOnMsgSendCompleteCallback(std::function<void(spConnection)> fun){
